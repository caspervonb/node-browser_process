...
const paths = {
  chrome: (function() {
    ...
    if (process.platform == 'linux') {
      paths = paths.concat(process.env['PATH'].split(path.delimiter).map(dirname => {
        return path.join(dirname, 'google-chrome');
      }));
    }
  ...
  chromium: (function() {
    ...
    if (process.platform == 'linux') {
      paths = paths.concat(process.env['PATH'].split(path.delimiter).map(dirname => {
        return path.join(dirname, 'chromium-browser');
      }));
    }
  ...
  electron: (function() {
    ...
    if (process.platform == 'darwin') {
      paths = paths.concat([
        path.resolve('node_modules/dist/Electron.app/Contents/MacOS/Electron'),
    ...
    if (process.platform == 'linux') {
      paths = paths.concat([
        path.resolve('node_modules/electron-prebuilt/dist/electron'),
      ]);
    }
  ...
  firefox: (function() {
    ...
    if (process.platform == 'linux') {
      paths = paths.concat(process.env['PATH'].split(path.delimiter).map(dirname => {
        return path.join(dirname, 'firefox');
      }));
    }
